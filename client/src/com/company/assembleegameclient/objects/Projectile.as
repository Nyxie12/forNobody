...
public class Projectile extends BasicObject {
    ...
    override public function draw(_arg1:Vector.<IGraphicsData>, _arg2:Camera, _arg3:int):void {
        ...
        var _loc10_:Boolean = !Parameters.data_.smartProjectiles?Boolean(this.projProps_.faceDir_):true;
        var _loc11_:Number = !_loc10_?Number(Number(this.angle_)):Number(Number(this.getDirectionAngle(_arg3)));
        var _loc12_:Number = !this.projProps_.noRotation_?Number(Number(_loc11_ - _arg2.angleRad_ + this.props_.angleCorrection_ + _local5)):Number(Number(_arg2.angleRad_ + this.props_.angleCorrection_));
        this.p_.draw(_arg1,this.staticVector3D_,_loc12_,_arg2.wToS_,_arg2,_local4);
        if (!Parameters.data_.noParticlesMaster && this.projProps_.particleTrail_) {
            ...
            for (; _local9 < 3; _local9++) {
                ...
                map_.addObj(new SparkParticle(100, this.projProps_.particleTrailColor_,
                        _local8, 0.5, RandomUtil.plusMinus(3), RandomUtil.plusMinus(3)), x_, y_);
            }
        }
    }
