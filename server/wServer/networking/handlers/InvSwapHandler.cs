...
namespace wServer.networking.handlers
{
    internal class InvSwapHandler : PacketHandlerBase<InvSwap>
    {
            ...
            int slotA, int slotB)
        {
            ...
            if (ValidateStackable(itemA, itemB, a, b, slotA, slotB))
            {
        ...
        private bool ValidateSlotSwap(Player player, IContainer conA, IContainer conB, int slotA, int slotB)
        {
            ...
            var availableSlot = player.Inventory.GetAvailableInventorySlot(gameData.Items[0x6895]);
            if (player.Inventory[0] == gameData.Items[0x6895] && player.Level < 20)
            {
                player.SendError("You cannot equip this item until you reach level [20]!");
                player.Inventory[availableSlot] = gameData.Items[0x6895];
                return false;
            }*/
        ...
        private bool ValidateStackable(Item itemA, Item itemB, Entity a, Entity b, int slotA, int slotB)
        {
            return itemA != null && itemB != null
                ...
                && slotA != slotB
                && IsSameKindOfItem(itemA, itemB);
        }

        private bool IsSameKindOfItem(Item itemA, Item itemB)
        {
